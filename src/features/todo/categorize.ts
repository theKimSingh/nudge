...
// MUST stay in sync with backend/lib/categorize.js — same KEYWORDS,
// same ERRAND_OVERRIDE, same PRIORITY, same algorithm. The voice path uses
// the JS mirror; the manual path uses this TS module.

import type { TaskCategory } from '@/src/types/database';

export const CATEGORY_IDS: readonly TaskCategory[] = [
  'meal',
  'exercise',
  'work',
  'study',
...
// First-match-wins priority. Order rationale documented in the plan;
// in short: highly specific categories (exercise, sleep, meal, health)
// run first; social runs above work so "Call mom" → social not work;
// study above work so "essay" beats "project"; errand above selfcare so
// "Pick up laundry" → errand not selfcare; selfcare last among real
// categories so its at-home keywords don't steal outdoor cases.
...
// Narrow override for the one case priority can't fix: "run" is in exercise,
// but "run to <X>" / "run errands" are clearly errand. Without this,
// "Run to the bank" → exercise.
...
const PATTERNS: Record<TaskCategory, RegExp[]> = Object.fromEntries(
  PRIORITY.map((cat) => [
    cat,
    KEYWORDS[cat].map(
...
) as Record<TaskCategory, RegExp[]>;

export function inferCategory(rawTitle: string): TaskCategory {
